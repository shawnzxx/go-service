...
// return sqlx.DB which is wrapper around sql.DB
...
// use for our readiness handler to call
func StatusCheck(ctx context.Context, db *sqlx.DB) error {
	...
	var pingError error
	// if ping success or exceed the deadline ctx
	...
	for attempts := 1; ; attempts++ {
		...
		// checks if the context ctx has been cancelled or has exceeded its deadline during each iteration
		if ctx.Err() != nil {
			return ctx.Err()
		}
	}

	// if ctx has been cancelled or has exceeded its deadline
